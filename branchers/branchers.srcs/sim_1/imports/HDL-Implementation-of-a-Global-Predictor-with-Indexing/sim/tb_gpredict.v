module tb_gpredict #(
    parameter TRACE_FILE_PATH = "branch_trace.txt")();
    integer file;
    reg clk;
    reg reset;
    reg predict_enable;
    ...
    initial begin
        ...
        // Open the branch trace file using parameter
        $display("Using trace file: %s", TRACE_FILE_PATH);
        file = $fopen(TRACE_FILE_PATH, "r");
        if (file == 0) begin
            $display("[ERROR] Failed to open file: %s", TRACE_FILE_PATH);
            $finish;
        end

        else begin
            $display("Successfully opened trace file: %s", TRACE_FILE_PATH);
        end
